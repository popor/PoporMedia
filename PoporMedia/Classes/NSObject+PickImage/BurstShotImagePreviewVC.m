...
- (void)viewDidLoad {
    ...
    _selectNum = 0;
    for (ImageDisplayEntity * entity in self.weakImageArray) {
        if (!entity.isIgnore) {
            _selectNum ++;
        }
    }
    ...
    [self customeSvScrollBlockAction:self.currentIndex];
    
    if (@available(iOS 11, *)) {
        self.imageSV.contentInsetAdjustmentBehavior = UIScrollViewContentInsetAdjustmentNever;
    }
}
...
- (void)setupSingleTapBlock {
    ...
    self.svScrollBlock = ^(int number) {
        [weakSelf customeSvScrollBlockAction:number];
    };
}
...
- (void)customeSvScrollBlockAction:(int)index {
    ImageDisplayEntity * entity = self.weakImageArray[index];
...
- (void)_selectButtonAction:(UIButton *)selectButton {
    ImageDisplayEntity * entity = self.weakImageArray[self.currentIndex];
